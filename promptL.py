    ...
    def forward(self, x_embed, y, task_id=-1, prompt_mask=None, layer_num= -1, cls_features=None):
        ...
        if self.prompt_pool:
            ...
            if self.training:
                ...
                if task_id > 0:
                    pkey_norm_old = self.l2_normalize(self.prompt_key[:self.lmax_list[-2]].clone(), dim=-1)
                    sim_old = torch.matmul(pkey_norm_old, x_embed_norm.t()) # pool_size, B or Pool_size, #class, B
                    sim_old = sim_old.t()
                    (simold_top_k, idx2) = torch.topk(sim_old, k=1, dim=1)
                    # print(idx2[:,0], y, " \npkey_norm_old shape ", pkey_norm_old.shape)
                    batched_key_old = pkey_norm_old[idx2[:,0]]
                    # print(batched_key_old.shape,x_embed_norm.shape)
                    orth = batched_key_norm * batched_key_old
                    # orth = torch.matmul(batched_key_norm, batched_key_norm.t()) 
                    # print("orth dim ",orth.shape)
                    out['reduce_sim2'] = torch.sum(orth) / x_embed_norm.shape[0] 
